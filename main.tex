...
a quantum algorithm with a given discrete gate set. In this
...
compare our approach against several other state-of-the-art
quantum circuit synthesis algorithms and show that are our
...
\begin{algorithm}[tb]
    \caption{
        ...
        Here $\Theta(k)$ represents intialization of an all zero array of size $k$,
        and the operator $A ~@_{i,j}~ B$ represents block multiplication of a matrix $B$ along rows $i,j$ of matrix $A$.
    }

    \label{fig:hurwitz_algo}

    \textbf{Input}: unitary matrix $A \in U(n)$ \\
    \textbf{Results}: real vectors $\theta,\phi \in [0, 2\pi)^k$, $\lambda \in [0, 2\pi)^n$
        ...
        \FOR {$i \in [0\dots(n-2)]$}
            \FOR {$j \in [n \dots (i-1)]$}
                ...
                \IF {$b > 0$}
                    ...
                    \ENDIF
                \ENDIF
                \STATE $U \gets A ~@_{i,j}~ G$
                \STATE $x \gets x + 1$
            \ENDFOR
        \ENDFOR
        \STATE $d \gets $ diagonal($A$)
        \STATE $d \gets d ~/~ |d|$
...
of a matrix $A \in U(n)$ into $n^2-1$ angles ranging from $0$ to $2\pi$ given by the algorithm in figure .
